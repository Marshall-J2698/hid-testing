...
{ // TODO: get a better sense of when and when not to use static!
    ...
    while (1) {
        if (xStreamBufferReceive(ascii_stream_buff,&ch,1,portMAX_DELAY) == 1) {
            if (ch == '\r' || ch == '\n') {
                if (pos > 0) {
                    msg_buff[pos] = '\0';
...
void app_main(void)
{
    ...
    */
    ...
    ascii_stream_buff = xStreamBufferCreate(10, 1);
    scan_read_created = xTaskCreatePinnedToCore(scan_reader_task,
                                                "scanner",
                                            4096,
                                            NULL,
                                        2,NULL,0);
